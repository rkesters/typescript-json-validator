...
if (process.platform !== 'win32') {
	...
	describe('Compare perivous schema gen to new', () => {
		test.each([{ file: 'ComplexExample.ts' }, { file: 'Example.ts' }, { file: 'DisjointUnionExample.ts' }, { file: 'OmitExample.ts' }])('$file', (tc) => {
			if (process.platform === 'win32') {
				return;
			}
			const oldSchema = parse([path.normalize(path.posix.join(path.posix.normalize(__dirname), `/build-parameters/src/${tc.file}`))], loadTsConfig()).getAllTypes();
			const newSchema = createParser(path.normalize(path.posix.join(path.posix.normalize(__dirname), `/build-parameters/src/${tc.file}`)), loadTsConfig(), {
				encodeRefs: true,
			}).getAllTypes();
