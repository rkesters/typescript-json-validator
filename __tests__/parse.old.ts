...
if (process.platform === 'win32') {
	...
	test('parse', () => {
		...
		expect(parse([__dirname + '/build-parameters/src/ComplexExample.ts'], loadTsConfig()).getAllTypes()).toMatchSnapshot();
	});
	...
	test('ajv', () => {
		...
		const parsed = parse([__dirname + '/build-parameters/src/ComplexExample.ts'], loadTsConfig(), {
			titles: true,
		});
