...
const { resolve } = path;
export default function parse(filenames: string[], tsConfig: TsConfigLoaderResult, settings: TJS.PartialArgs = {}) {
	filenames = filenames.map((f) => resolve(f));
	const program = TJS.getProgramFromFiles(filenames, tsConfig.tsConfig.compilerOptions);
