...
const complexPath = path.join(__dirname, 'build-parameters/src/ComplexExample.ts');
const complexPathValidator = path.join(__dirname, '/output/ComplexExample.validator.ts');

const complexPathValidatorUsage = path.join(__dirname, '/output/ComplexExample.usage.ts');
...
async function generateValidator() {
	...
	writeFileSync(complexPathValidator, printTypeCollectionValidator(parser, symbols, schema, '../build-parameters/src/ComplexExample', tsConfig.tsConfig));
...
describe('Given Validator', () => {
	...
	test('validateValue', () => {
		...
		expect(() => validate.validate('MyEnum')(42)).toThrowErrorMatchingInlineSnapshot(`"Invalid MyEnum: MyEnum should be equal to one of the allowed values"`);
	});
