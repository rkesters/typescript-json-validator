	...
	},
	{
		...
		types: [
			...
			},
			{
				...
				validations: [{ value: { query: { id: 'x', value: 'y' }, params: { e: 42 } }, toBe: false }],
			},
		],
	},
	...
	tc.types.forEach((type) => {
		...
		type.validations.forEach((v) => {
			if (validator(v.value) !== v.toBe) {
				console.log(`Failed validation ${JSON.stringify(v.value)}, expected: ${v.toBe}`);
			}
			expect(validator(v.value)).toBe(v.toBe);
			if (!v.toBe) {
				expect(ajv.errorsText(validator.errors, { dataVar: 'x' })).toMatchSnapshot();
			}
		});
	});
});
...
test('ajv', () => {
	...
	expect(ajv.errorsText(validateMyEnum.errors, { dataVar: 'x' })).toMatchInlineSnapshot(`"x should be equal to one of the allowed values"`);
