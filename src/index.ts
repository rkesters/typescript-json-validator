...
function makeOutput(outputFilePath: string) {
	...
	if (!existsSync(dir)) {
		...
		if (!isRelative) {
			throw new Error(`Output (${outputFilePath}) directory must be at or below the current directory (${parent})`);
		}
		mkdirSync(rel);
	}
}
...
export default function run(args?: string[]) {
	...
	files.forEach(({ fileName, typeName }) => {
		const outputFileName = output ?? fileName.replace(/\.tsx?$/, '.validator.ts');
		...
		if (typeName) {
			...
			validator = printSingleTypeValidator(
				...
				`${getRelativePath(fileName, outputFileName)}/${basename(fileName, /\.ts$/.test(fileName) ? '.ts' : '.tsx')}`,
				tsConfig,
				options.ajv,
			);
		} else {
