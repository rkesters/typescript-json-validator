...
import { JSONSchema7Definition as Definition, JSONSchema7, JSONSchema7TypeName } from 'json-schema';
...
function processAnyOf(types: Definition[], definitions: DefinitionMap): JSONSchema7 {
...
export function isDefinition(value: TJS.DefinitionOrBoolean | undefined | null): value is JSONSchema7 {
...
function getCandidates(type: TJS.DefinitionOrBoolean | undefined) {
	...
	return required.filter((key) => {
		...
		return (property.type === 'string' || property.type === 'number') && property.enum && property.enum.length === 1;
	});
}
function getType(type: Definition | undefined, key: string): JSONSchema7TypeName | JSONSchema7TypeName[] | undefined {
...
function getValue(type: TJS.DefinitionOrBoolean | undefined, key: string): JSONSchema7Type {
	if (!type) return '';
	if (!isDefinition(type)) return '';
	const property: TJS.Definition = type.properties![key] as unknown as TJS.Definition;
