...
function isKoaType(typeDefinition: TJS.DefinitionOrBoolean) {
	...
	return (
		...
		Object.keys(typeDefinition.properties).every((property) => KoaProperties.includes(property))
	);
}
const KoaProperties = ['params', 'query', 'body'];
export function printTypeCollectionValidator(parser: IParser, symbols: string[], schema: TJS.Definition, relativePath: string, tsConfig: Tsconfig, options: Ajv.Options = {}) {
	...
	return [
		...
		...(koaTypes.length ? [t.VALIDATE_KOA_REQUEST_FALLBACK, t.VALIDATE_KOA_REQUEST_IMPLEMENTATION] : []),
 ...
 */
export function printSingleTypeValidator(parser: IParser, typeName: string, schema: TJS.Definition, relativePath: string, tsConfig: Tsconfig, options: Ajv.Options = {}) {
	return [
		t.TSLINT_DISABLE,
		t.GENERATED_COMMENT,
		t.IMPORT_INSPECT,
