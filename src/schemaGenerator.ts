...
function createBaseConfig(filename: string, tsconfig: TsConfigLoaderResult, settings: tjs.Config = {}): tjs.Config {
	return {
		...settings,
		path: filename,
...
export function createParser(filename: string, tsconfig: TsConfigLoaderResult, settings: tjs.Config = {}): IParser {
	...
	return {
		getTypeExport(name: string): string {
			...
			src?.forEachChild((c) => {
				...
				const symbolsForNode = program.getTypeChecker().getSymbolsInScope(c, syntaxToSymbolKind[c.kind] ?? 0);
				...
				if (symbolType.getEscapedName().toString() === name) {
					exportName = program.getTypeChecker().getExportSymbolOfSymbol(symbolType).getEscapedName().toString();
				}
			});
			if (!exportName) return '';
			return exportName;
